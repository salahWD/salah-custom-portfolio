...
window.onload = function () {
  ...
  document.addEventListener("click", function (e) {
    if (e.target != headerBtn && !Array.of(...headerBtn.children).includes(e.target)) {
  ...
  pages.forEach(page => {
    page.addEventListener("scroll", () => {
      ...
      if (header.classList.contains("active") || pagesCont.classList.contains("active")) {
        header.classList.remove("active");
        pagesCont.classList.add("wide");
      }
  ...
  if (localStorage.getItem('mood') != null) {
    if (localStorage.getItem('mood') == 2) {
      document.body.className = "dark";
      document.getElementById("mood-toggler").className = "fa fa-sun";
    }
  }
